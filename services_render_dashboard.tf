...
locals {
  ...
  _services_render_dashboard_service_cards = flatten([
  ...
  _services_render_dashboard_sort_index = {
    for dashboard_card in concat(local._services_render_dashboard_service_cards, local._services_render_dashboard_server_cards) : dashboard_card.sort => dashboard_card
  }

  _services_render_dashboard_sorted_by_group = {
    for card in [
      for sort_key in sort(keys(local._services_render_dashboard_sort_index)) :
      local._services_render_dashboard_sort_index[sort_key]
  ...
  _services_render_dashboard_sorted_server_groups = [
  ...
  _services_render_dashboard_union_groups = concat(
    ...
    ["Providers"],
    local._services_render_dashboard_sorted_server_groups,
  )

  services_render_dashboard_view = {
    layout = [
      for group in local._services_render_dashboard_union_groups : {
        (group) = merge(
          {
            columns = 2
            style   = "row"
    ...
    services = [
      for group in local._services_render_dashboard_union_groups : {
        (group) = try(local._services_render_dashboard_sorted_by_group[group], [])
      }
      if group != "Providers"
    ]
  }
