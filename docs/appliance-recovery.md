...
`hass.mbk.excloo.net` in the Servers vault.
...
existing Tailscale client; the managed key under `mandu.mbk.excloo.net` is for
...
recovery key under `ramen.mbk.excloo.net` if the gateway installation uses it.
