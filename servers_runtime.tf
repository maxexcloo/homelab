        ...
        runtime = {
          ...
          attributes = merge(
            ...
            server.features.object_storage ? {
              object_storage_access_key_id = module.server_object_storage.items[server_key].access_key_id
              object_storage_bucket        = module.server_object_storage.items[server_key].bucket
              object_storage_endpoint      = module.server_object_storage.items[server_key].endpoint
            } : {},
          )
          ...
          credentials = merge(
            ...
            },
            {
              for field_name, field in server.credentials.fields : field_name => sensitive(try(coalesce(
                ...
                try(module.server_credentials.values["${server_key}-${field_name}"], null),
              ), ""))
              if field.mode == "rw"
            },
            ...
            server.features.mail ? {
              mail_password = module.server_resend.tokens[server_key]
            } : {},
            server.features.object_storage ? {
              object_storage_secret_access_key = module.server_object_storage.items[server_key].secret_access_key
            } : {},
            (
              ...
              password      = module.server_credentials.passwords[server_key].value
              password_hash = module.server_credentials.passwords[server_key].hash
            } : {},
            ...
            merge({}, [
              for credential_name, generator in server.credentials.generated : {
                "${credential_name}_certificate" = module.server_credentials.x509["${server_key}-${credential_name}"].certificate
                "${credential_name}_private_key" = module.server_credentials.x509["${server_key}-${credential_name}"].private_key
              }
