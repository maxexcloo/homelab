...
resource "terraform_data" "services_validation" {
  ...
  lifecycle {
    ...
    precondition {
      condition     = length(local._services_validation_monitoring_items_missing) == 0
      error_message = "Monitored services must have a 1Password item: ${join(", ", local._services_validation_monitoring_items_missing)}"
    }
