locals {
  ...
  _servers = {
    for k, v in local._servers_raw : k => merge(
      var.server_defaults,
      v,
      {
        fqdn = k
        id   = k
        slug = "${try(v.region, var.server_defaults.region)}-${k}"
      }
    )
  }
  ...
  servers = {
    for k, v in local._servers : k => merge(
      v,
      {
        fqdn_external = "${v.fqdn}.${local.defaults.domain_external}"
        fqdn_internal = "${v.fqdn}.${local.defaults.domain_internal}"
        password_hash = try(htpasswd_password.server[k].sha512, "")
        ssh_keys      = data.github_user.default.ssh_keys

        private_address = try(local.unifi_clients[v.slug].local_dns_record, null)
        private_ipv4    = try(local.unifi_clients[v.slug].fixed_ip, null)
