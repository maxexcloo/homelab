      ...
      for rule in rules : merge(
        rule,
        {
          ...
          expression = sensitive(templatestring(
            rule.expression,
            nonsensitive(local.services_template_contexts[rule.service_key]),
          ))
        },
      )
    ]
...
resource "cloudflare_dns_record" "all" {
  ...
  comment  = local.defaults.organisation.managed_comment
...
resource "cloudflare_ruleset" "rate_limiting" {
  ...
  description = local.defaults.organisation.managed_comment
  kind        = "zone"
  name        = "default"
