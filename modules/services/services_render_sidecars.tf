...
locals {
  ...
  services_render_sidecar_inputs = {
    for file_input in flatten([
      for service_key, service in {
        for service_key, service in local.services_model : service_key => service
        if service.identity.service != null
        } : [
        for file_path in fileset(path.root, "templates/services/${service.identity.service}/**") : {
      ...
      ]) : "${file_input.stack}/${file_input.rel_path}" => merge(
      file_input,
      {
        content_type = (
